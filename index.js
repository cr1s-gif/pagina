...
app.get('/cart', authMiddleware, async (req, res) => {
  const user_id = req.user.id;
try{
    const query = `SELECT p.id, p.name,   c.quantity, SUM(c.quantity * p.price) as price, SUM(c.quantity * p.price) OVER () as total
  FROM cart c
  JOIN products p ON c.product_id = p.id
  WHERE c.user_id = $1
  GROUP BY p.id, p.name, c.quantity, p.price
    `;
...
app.listen(3000, () => console.log('mostrando la pagina kla'));
